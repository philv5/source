...
void
increRefcount(uint pa)
{
  acquire(&kmem.lock);
  kmem.refcount[pa >> PGSHIFT]++;
  release(&kmem.lock);
}

void
decreRefcount(uint pa)
{
  acquire(&kmem.lock);
  kmem.refcount[pa >> PGSHIFT]--;
  release(&kmem.lock);
