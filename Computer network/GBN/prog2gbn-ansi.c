...
#define WINDOWSIZE 1 /* GBN window size */
...
void A_input(struct pkt packet)
{
  ...
  if ( (CheckCorrupted(packet) == FALSE) && (packet.acknum != NACK) ) {
       ...
       while ( (msgnum!=0) && (nextseqnum<base+WINDOWSIZE) ) {
        ...
        /* NACK와 time out packet 은 모두 A_timerinterrupt에서 resent */
}
...
void tolayer3(int AorB,struct pkt packet) /* A or B is trying to stop timer */
{
 ...
 if ( x < lossprob)  {
      ...
      expectedseqnum = expectedseqnum-1;        
      if (TRACE>0)    
	printf("          TOLAYER3: packet being lost. rand=%f loss= %f\n", x, lossprob);
      return;
    }  
	  ...
	  break;                        /* all done with simulation */
        if (eventptr->evtype == FROM_LAYER5 ) {
            ...
            for (i=0; i<strlen(my_name); i++)  
               msg2give.data[i] = my_name[i];
            for (i=strlen(my_name); i<20; i++)  
